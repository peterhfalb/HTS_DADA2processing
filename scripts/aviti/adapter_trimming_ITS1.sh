#!/bin/bash
# Aviti ITS1 Adapter and Primer Trimming (3-step: Trimmomatic, cutadapt, Trimmomatic)
set -euo pipefail
...
[ -f "$AVITI_ADAPTERS" ] && [ -f "$AVITI_PRIMERS" ] || {
  echo "ERROR: Aviti adapter FASTA files not found in $SCRIPT_DIR"
  exit 1
}

echo "ITS1 Aviti trimming: Step 1 (Trimmomatic adapters)..."
...
echo "Step 2 (cutadapt ITS1F/ITS2 primers)..."
for i in 01_adapter/*_R1_001.paired.fastq.gz; do
  basename="${i##*/}"
  echo "cutadapt --cores 8 --pair-filter=any --minimum-length 100 --discard-untrimmed -g CTTGGTCATTTAGAGGAAGTAA -G GCTGCGTTCTTCATCGATGC -o 02_filtered/${basename//_R1_001.paired.fastq.gz/_R1_001.fastq.gz} -p 02_filtered/${basename//_R1_001.paired.fastq.gz/_R2_001.fastq.gz} ${i} ${i//_R1_/_R2_} > 02_filtered/cutadapt.${basename//_R1_001.paired.fastq.gz/.log.txt}" >> run_cutadapt.sh
done
chmod +x run_cutadapt.sh && parallel < run_cutadapt.sh

echo "Step 3 (Trimmomatic primers)..."
...
echo "ITS1 Aviti trimming complete."
