#!/bin/bash
# Aviti 18S-AMF (Arbuscular Mycorrhizal Fungi) Adapter and Primer Trimming (3-step)
set -euo pipefail
...
for i in 01_adapter/*_R1_001.paired.fastq.gz; do
  basename="${i##*/}"
  echo "cutadapt --cores 8 --pair-filter=any --minimum-length 100 --discard-untrimmed -g CAGCCGCGGTAATTCCAGCT -G GAACCCAAACACTTTGGTTTCC -o 02_filtered/${basename//_R1_001.paired.fastq.gz/_R1_001.fastq.gz} -p 02_filtered/${basename//_R1_001.paired.fastq.gz/_R2_001.fastq.gz} ${i} ${i//_R1_/_R2_} > 02_filtered/cutadapt.${basename//_R1_001.paired.fastq.gz/.log.txt}" >> run_cutadapt.sh
done
...
echo "18S-AMF Aviti trimming complete."
